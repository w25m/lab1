...
public:
	CircleAsteroid(int w, int h) : Asteroid(w, h) { baseDamage = 20; }
	void Draw() const override {
		Renderer::Instance().DrawCircle(transform.position, GetRadius());
	}
};

// Shape selector
enum class AsteroidShape { TRIANGLE = 3, SQUARE = 4, PENTAGON = 5, CIRCLE = 6, RANDOM = 0 };
...
enum class WeaponType { LASER, BULLET, SPREAD, COUNT };
class Projectile {
public:
...
public:
	...
	switch (wt) {
		...
		case WeaponType::SPREAD: return fireRateSpread;
...
float GetSpacing(WeaponType wt) const {
	switch (wt) {
		...
		case WeaponType::SPREAD: return spacingSpread;
	...
	void Run() {
		...
		while (!WindowShouldClose()) {
			...
			if (IsKeyPressed(KEY_FOUR)) {
				currentShape = AsteroidShape::CIRCLE;
			}
			...
			if (player->IsAlive() && IsKeyDown(KEY_SPACE)) {
				...
				if (currentWeapon == WeaponType::SPREAD) {
					Vector2 velCenter = { 0.f, -projSpeed };
					Vector2 velLeft = Vector2Rotate(velCenter, DEG2RAD * -20);  
					Vector2 velRight = Vector2Rotate(velCenter, DEG2RAD * 20);  

					projectiles.emplace_back(p, velCenter, 8, WeaponType::SPREAD);
					projectiles.emplace_back(p, velLeft, 8, WeaponType::SPREAD);
					projectiles.emplace_back(p, velRight, 8, WeaponType::SPREAD);
				}
				else {
			...
			// Render everything
			{
				...
				const char* weaponName = (currentWeapon == WeaponType::LASER) ? "LASER" :(currentWeapon == WeaponType::BULLET) ? "BULLET" :"SPREAD";
